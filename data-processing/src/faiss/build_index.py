...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Build FAISS index from .npy embeddings"
    )
    parser.add_argument(
        "--input_dir",
        required=True,
...
def main():
    ...
    for npy_file in tqdm(npy_files, desc="Loading .npy files"):
        try:
            ...
            # Build id-to-metadata mapping efficiently
            faiss_ids.extend(np.arange(global_id, global_id + num_frames).tolist())
            video_ids.extend([video_name] * num_frames)
            frame_indices.extend(np.arange(num_frames).tolist())
    ...
    mapping_df = pd.DataFrame(
        {"faiss_id": faiss_ids, "video_id": video_ids, "frame_idx": frame_indices}
    )
