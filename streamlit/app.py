...
def render_result_card(result: dict, idx: int) -> str:
    video_id      = esc(result.get("video_id") or "—")
    frame_id      = esc(result.get("frame_id", "—"))
    ocr_raw       = result.get("ocr_text") or ""
    azure_url     = result.get("azure_url") or ""
    youtube_link  = result.get("youtube_link") or ""
    score         = float(result.get("score", 0) or 0)
    branch        = esc(result.get("branch", ""))
    evidence      = [esc(x) for x in (result.get("evidence", []) or [])]

    ocr_text = esc(ocr_raw[:100])

    bar_pct = min(int(score * 2000), 100)
    branch_color = {"agentic": "#6366f1", "heuristic": "#10b981", "fused": "#f59e0b"}.get(result.get("branch", ""), "#6b7280")

    if azure_url:
        safe_url = html.escape(azure_url, quote=True)
        img_html = f'<img src="{safe_url}" class="card-img" onerror="this.style.display=\'none\'; this.insertAdjacentHTML(\'afterend\', \'<div class=&quot;card-img-placeholder&quot;><span>Image load failed</span></div>\');">'
    else:
        img_html = '<div class="card-img-placeholder"><span>No Image</span></div>'

    yt_html = ""
    if youtube_link:
        safe_yt = html.escape(youtube_link, quote=True)
        yt_html = f'<a href="{safe_yt}" target="_blank" class="yt-btn">▶ YouTube</a>'

    ocr_html = f'<div class="card-ocr"><span class="ocr-label">OCR</span>{ocr_text}</div>' if ocr_raw else ""
    evidence_html = (
        f'<div style="font-size:0.7rem;color:#9ca3af;margin-top:4px;">Sources: {", ".join(evidence)}</div>'
        if evidence else ""
    )

    return f"""<div class="result-card" style="animation-delay: {idx * 0.06}s">
...
def render_latency_badge(latency: dict) -> str:
    ...
    return f"""<div class="latency-bar">
...
if "results_data" in st.session_state:
    ...
    # Header row (no <a> tags, safe for st.markdown)
...
{render_latency_badge(data.get("latency_ms", dict()))}""", unsafe_allow_html=True)

    # Cards grid — use components.html so <a> tags render correctly
    cards_html = '<div class="results-grid">'
    for i, result in enumerate(data["results"]):
        cards_html += render_result_card(result, i)
    cards_html += "</div>"

    # Wrap with full HTML doc including the app CSS for the iframe
...
{_app_css}
    ...
    # Estimate iframe height: ~280px per card row, 2 cards per row
    n_results = len(data["results"])
    rows = (n_results + 1) // 2
    iframe_height = max(rows * 320, 400)
...
if search_query:
    ...
    spinner_placeholder.markdown("""<div class="custom-loader-container">
